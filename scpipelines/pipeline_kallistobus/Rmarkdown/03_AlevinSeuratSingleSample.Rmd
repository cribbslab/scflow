...
```{r}
txi.control <- tximport("salmon.dir/Adam-1_S2/alevin/quants_mat.gz", type="alevin")
...
control <- CreateSeuratObject(counts = d, project = "CTRL")
control$stim <- "CTRL"
...
```{r}
control <- subset(control, subset = nFeature_RNA > 10 & nFeature_RNA < 4000 & percent.mt < 15)
...
```{r}
control <- FindVariableFeatures(control, selection.method = "disp", nfeatures = 1000)
...
FeaturePlot(control, features = c("TNNC2", "ENO3"), split.by = "stim",
            order = TRUE)
```
...
cluster1.markers <- FindMarkers(control, ident.1 = 0, min.pct = 0.25)
...
```{r}
# Markers 0, 1, 2 - skeletal muscles
FeaturePlot(control, features = c("MYL1", "MYH2"), split.by = "stim",
            order = TRUE)


# Markers 3 - extracellular vesicles?
FeaturePlot(control, features = c("CD63", "YIPF4"), split.by = "stim",
            order = TRUE)
```
...
# Lebel cells using singleR

```{r}
bpen.se <- BlueprintEncodeData()
counts.data <-  GetAssayData(control, slot = "data")
...
control <- AddMetaData(
  object = control,
  metadata = annot$pruned.labels,
  col.name = 'cell-labels'
)


DimPlot(control, reduction = "umap", group.by = "cell-labels")
```
