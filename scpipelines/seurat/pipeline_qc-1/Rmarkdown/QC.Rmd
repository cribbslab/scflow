...
tr2g <- read_tsv("tr2g.tsv", col_names = c("transcript", "gene", "gene_symbol")) %>% 
      dplyr::select(-transcript) %>%
      distinct()
    
tr2g$gene_v2 <- sub("\\..*", "", tr2g$gene)
...
for (i in 1:length(file_list)){
    ...
    rownames(res_mat) <- tr2g$gene_v2[match(rownames(res_mat), tr2g$gene)]
