...
for (i in sample_files){
  ...
  so <- CreateSeuratObject(res_mat, min.cells=20, min.features=ini$so_filter) %>% 
...
Specify some ggplot themes for use later
```{r setup}
...
for (i in sample_files){
  ...
  # rename nCount_RNA to nUMI in metadata
  colnames(metadata)[2] <- "nUMI"
  # rename nFeature_RNA to nGene
...
Add details of reads from mitochondrial genes to the metadata
...
for (i in sample_files){
  ...
  so_assay <- AddMetaData(so[["RNA"]], joined_ensembl_hgnc$hgnc_symbol, col.name = "hgnc_symbols")
  # Can't add as normal metdata, have to add to assay instead :@
  so[["RNA"]] <- so_assay # Symbols stored in so@assays$RNA@meta.features$hgnc_symbols
...
idx <- which(metadata$nUMI > ini$UMI_filter)
...
# Visualize the number of cell counts per sample {.tabset .tabset-fade}
...
name<- paste0("QC_Figures.dir/GenesDetectedBoxplot_", i, ".eps")
postscript(name)
plot(plt)
dev.off()
cat('\n\n')
...
name<- paste0("QC_Figures.dir/MitoRatio_", i, ".eps")
postscript(name)
print(plt)
dev.off()
cat('\n\n')
