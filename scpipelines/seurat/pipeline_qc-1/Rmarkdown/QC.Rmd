...
TO DO move the addition of patient metadata to the next pipeline that is specific to the samples being analysed. 
...
### Add patient metadata to each sample
...
- sequencing date
- disease status
Modify to add any other required fields. 
...
for (i in 1:length(so)){
    ...
    #cat("\n")
    sample_name <- str_sub(so[[i]]@project.name, 1, 7)
    info <- metadata %>% filter(Patient == sample_name)
    
    so[[i]]@meta.data$sex <- info$Sex
    so[[i]]@meta.data$diagnosis <- info$Diagnosis
    so[[i]]@meta.data$surgeon <- info$Surgeon
    so[[i]]@meta.data$affected_side <- info$Affected.side
    so[[i]]@meta.data$surgical_procedure <- info$Surgical.procedure
    so[[i]]@meta.data$age <- info$Age
    so[[i]]@meta.data$sequencing_date <- info$Sequencing.date
    so[[i]]@meta.data$disease_status <- info$Disease.status
...
for(i in 1:length(so)){
    ...
    # run emptydrops
    set.seed(1)
    out <- DropletUtils::emptyDrops(assay(sce[[i]], "counts"), lower = 50)
