...
combined <- combined %>% dplyr::select(ensembl, cluster, p.adj, p_val, p.adj.bonferroni, avg_log2FC, pct.1, pct.2)

# Rearrange and select columns
combined_padj_order <- combined %>% dplyr::arrange(cluster, p.adj, desc(abs(avg_log2FC)))
combined_logfc_order <- combined %>% dplyr::arrange(cluster, desc(abs(avg_log2FC)), p.adj)
