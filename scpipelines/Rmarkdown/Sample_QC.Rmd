...
#mart <-biomaRt::useMart(biomart = "ENSEMBL_MART_ENSEMBL", dataset = ensembl_data, host = "www.ensembl.org", ensemblRedirect = FALSE)
#sce <- biomaRt::getBM(sce, filters = "ensembl_gene_id", attributes = c("ensembl_gene_id", symbol, "chromosome_name"), mart = mart)

#sce <- scater::getBMFeatureAnnos(sce, filters = "ensembl_gene_id", attributes = c("ensembl_gene_id", symbol, "chromosome_name"), dataset = ensembl_data)
...
plotColData(sce, x = "total_features_by_counts", y = "pct_counts_MT") + geom_hline(yintercept = mt_threshold, col ="red") + geom_vline(xintercept = transcript_thresh, col = "red")
```
...
if(mad){
ggplot(as.data.frame(colData(sce)), aes(rownames(colData(sce)), pct_counts_MT)) +
  geom_point() +
  geom_hline(
...
plotColData(sce.pass, x = "total_features_by_counts", y = "pct_counts_MT")+  geom_hline(yintercept =10, col ="red") + geom_vline(xintercept = 1000, col = "red")
