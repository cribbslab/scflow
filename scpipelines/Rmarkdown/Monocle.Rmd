...
gene_marker <- opts$monocle$gene_marker
max_components <- opts$monocle$max_components
num_dim <- opts$monocle$num_dim
num_clusters <- opts$monocle$num_clusters
```
...
seurat <- importCDS(readRDS("alevin_combined_seurat.rds"))

seurat <- estimateSizeFactors(seurat)
seurat_temp <- try(estimateDispersions(seurat))
if(isS4(seurat_temp)){
  seurat <- seurat_temp
}
...
seurat <- reduceDimension(seurat, max_components = max_components, num_dim = num_dim,
                          reduction_method = 'tSNE', verbose = T)
seurat <- clusterCells(seurat, num_clusters = 4)
plot_cell_clusters(seurat, 1, 2, color = "stim",
                   markers = gene_marker) # Can change to list of markers
...
seurat <- clusterCells(seurat, num_clusters = num_clusters)
plot_cell_clusters(seurat, 1, 2, color = "Cluster")

postscript(paste0(gene_marker, "_total.eps", sep = ""))
plot_cell_clusters(seurat, 1, 2, color = "Cluster", markers = gene_marker) +
  facet_wrap(~stim)
dev.off()
