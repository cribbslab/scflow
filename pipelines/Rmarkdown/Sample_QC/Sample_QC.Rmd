...
```{r}
sce <- readRDS("R.dir/hgmm_100_S1.rds")
...
filter_by_detected_genes <- (sce$total_features_by_counts < 50)
```

# ERCCs and MT genes

A particularly useful plot for cell-level QC involves percentage of expression in feature controls against the total number of expressed features. These two metadata variables can be plotted against each other as shown below. 
...
```{r ercc_cont, }
# need to parameterise to state that it is onyl active if ERCC present
metric <- sce.sc$pct_counts_ERCC
scater.ERCC.M <- median(metric)
scater.ERCC.MAD <- mad(metric)
scater.ERCC.outliers <- metric > scater.ERCC.M + 10 * scater.ERCC.MAD
table(scater.ERCC.outliers)

outlier$ERCC <- metric > scater.ERCC.M + 10 * scater.ERCC.MAD
```


```{r ercc_plot_QC,  echo=FALSE}
ggplot(as.data.frame(colData(sceset)), aes(rownames(colData(sceset)), pct_counts_ERCC)) +
  geom_point(aes(colour = Lane, shape = Condition)) +
  geom_hline(
    yintercept = scater.ERCC.M + 10 * c(-1,1) * scater.ERCC.MAD,
    colour = "red", linetype = 2:1
  ) +
...
#postscript("Figures.dir/cells_passing_filter_MCF7.eps")
draw(hm)
#dev.off()
```
