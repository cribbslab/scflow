...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
rowData(sce.filtered)$feature_symbol <- rownames(sce.filtered)
sce.filtered <- sce.filtered[!duplicated(rowData(sce.filtered)$feature_symbol), ]
# Turn on gene_filter=TRUE if your data requires it
sce.filtered <- sc3(sce.filtered, ks = 5, biology = TRUE)
```
